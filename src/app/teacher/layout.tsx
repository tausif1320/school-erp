    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-200 font-sans flex selection:bg-indigo-500/30">
      ...
      `}>
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-white/5">
          ...
          <button onClick={() => setIsMobileOpen(false)} className="lg:hidden text-zinc-400 hover:text-white"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-3 space-y-1.5 custom-scrollbar">
          {NAV_ITEMS.map((item, idx) => (
             <SidebarItem 
               ...
               onNavigate={closeMobileMenu} // Pass handler
             />
          ))}
        </div>
        ...
        <div className="p-4 border-t border-white/5 bg-zinc-900/30">
          <div className={`flex items-center gap-3 ${!sidebarOpen && 'lg:justify-center'}`}>
            <div className="w-9 h-9 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center shrink-0"><UserCircle className="w-5 h-5 text-zinc-400" /></div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-black/20">
        <header className="h-20 sticky top-0 z-30 bg-zinc-950/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 lg:px-8">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsMobileOpen(true)} className="lg:hidden p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-white/5"><Menu className="w-6 h-6" /></button>
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden lg:block p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-white/5 transition-colors"><Menu className="w-5 h-5" /></button>
          </div>
          <div className="flex items-center gap-4">
             <div className="relative group hidden md:block">
               ...
               <button onClick={() => { router.push('/teacher/scan'); closeMobileMenu(); }} className="relative flex items-center gap-2 px-4 py-2 bg-zinc-900 rounded-lg text-sm font-medium text-white border border-white/10 hover:bg-zinc-800 transition-all"><QrCode className="w-4 h-4 text-emerald-400" /><span>Scan ID</span></button>
             </div>
             <div className="w-px h-6 bg-white/10 mx-1"></div>
             <LogoutButton />
          </div>
        </header>
        <div className="p-4 lg:p-8 flex-1 overflow-x-hidden animate-fade-in-up">{children}</div>
      </main>
    </div>
  );
}
  ...
  return (
    <Link 
      ...
      className={`
        ...
        ${isActive ? 'bg-emerald-600/10 text-emerald-400 border border-emerald-500/20' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200 border border-transparent'}
      `}
    >
