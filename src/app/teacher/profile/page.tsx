...
function Section({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/5 p-8 rounded-3xl shadow-xl transition-all hover:border-white/10 group">
    ...
    <div className="space-y-2 group">
      ...
      <div className="relative">
        ...
        {edit && !locked ? (
          ...
          <div className={`
            w-full rounded-xl py-2.5 pl-11 pr-4 text-sm font-medium cursor-default
            ${locked 
              ? 'bg-indigo-500/5 border border-indigo-500/10 text-indigo-200/80' 
              : 'bg-white/5 border border-transparent text-zinc-300'}
          `}>
...
export default function TeacherProfile() {
  ...
  return (
    <div className="animate-fade-in-up pb-24 md:pb-10 max-w-5xl mx-auto space-y-8">
      ...
      {/* --- HEADER --- */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 sticky top-20 z-40 bg-zinc-950/80 backdrop-blur-md py-4 -my-4 px-1 md:-mx-1">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
            My Profile
            {isEditing && <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full border border-indigo-500/30">Editing</span>}
          </h1>
        ...
        {!isEditing ? (
          <button 
            ...
            className="group flex items-center gap-2 px-6 py-3 bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-white rounded-xl font-medium transition-all shadow-lg hover:shadow-xl hover:border-white/20"
          >
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <button 
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-zinc-900 border border-white/10 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-xl font-medium transition-all"
            >
      ...
      <div className="grid grid-cols-1 gap-8 pt-4">
        ...
        <Section title="Personal Information" icon={<User className="w-5 h-5" />}>
          ...
          <FieldGroup label="Email Address" name="email" value={formData.email} onChange={handleChange} edit={false} locked icon={<Mail className="w-4 h-4" />} />
