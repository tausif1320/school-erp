    ...
    <div className="space-y-8 animate-fade-in-up pb-20 md:pb-10 perspective-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 bg-zinc-900/40 backdrop-blur-xl border border-white/5 p-2 rounded-2xl relative z-10 shadow-lg">
        <div className="col-span-1 md:col-span-5 relative group">
          ...
          <input placeholder="Search by Name or ID..." className="w-full bg-black/40 border border-white/5 rounded-xl py-3 pl-11 pr-4 text-sm text-white focus:bg-black/60 focus:border-indigo-500/50 outline-none transition-all placeholder:text-zinc-600 font-medium" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      ...
      <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-3xl overflow-hidden shadow-2xl flex flex-col relative">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse">
            ...
            <thead className="bg-black/40 border-b border-white/5">
              <tr>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[140px]">Admission ID</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest min-w-[200px]">Student Name</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[120px]">Class</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[120px]">Section</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[140px]">Year</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[140px]">Status</th>
                <th className="px-6 py-5 text-[10px] font-bold uppercase text-zinc-500 tracking-widest w-[160px] text-right">Actions</th>
              </tr>
            </thead>
              ...
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="px-6 py-4"><div className="h-3 w-32 bg-white/5 rounded"></div></td>
                    <td className="px-6 py-4"><div className="h-3 w-8 bg-white/5 rounded"></div></td>
                    <td className="px-6 py-4"><div className="h-3 w-8 bg-white/5 rounded"></div></td>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-24 text-center text-zinc-500">
                ...
                paginatedStudents.map((s) => (
                  <tr key={s.id} className="group hover:bg-white/[0.03] transition-colors">
                    <td className="px-6 py-4">
                      <span className="font-mono text-xs text-zinc-400 bg-white/5 px-2 py-1 rounded border border-white/5">{s.admission_number}</span>
                    </td>
                    <td className="px-6 py-4">
                      <Link href={`/admin/students/${s.id}`} className="font-bold text-sm text-white hover:text-indigo-400 transition-colors">
                        {s.full_name}
                      </Link>
                    </td>
                    <td className="px-6 py-4 text-sm text-zinc-300">{s.class}</td>
                    <td className="px-6 py-4 text-sm text-zinc-400">{s.section || '-'}</td>
        ...
        <div className="p-4 border-t border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-md">
          <p className="text-xs text-zinc-500 font-medium">
            Showing <span className="text-white">{Math.min((currentPage - 1) * rowsPerPage + 1, filteredStudents.length)} - {Math.min(currentPage * rowsPerPage, filteredStudents.length)}</span> of <span className="text-white">{filteredStudents.length}</span>
          </p>
          ...
          <div className="bg-zinc-900 border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-up">
            ...
            <div className="p-6 space-y-4">
              <InputGroup 
                ...
                onChange={(val: string) => setEditingStudent(prev => prev ? {...prev, full_name: val} : null)} 
              />
              <div className="grid grid-cols-2 gap-4">
                 <InputGroup 
                   ...
                   onChange={(val: string) => setEditingStudent(prev => prev ? {...prev, class: val} : null)} 
                 />
                 <InputGroup 
                   label="Section" 
                   icon={<Filter className="w-4 h-4" />} 
                   value={editingStudent.section ?? ''} 
                   onChange={(val: string) => setEditingStudent(prev => prev ? {...prev, section: val} : null)} 
                 />
              </div>
              <InputGroup 
                label="Academic Year" 
                ...
                onChange={(val: string) => setEditingStudent(prev => prev ? {...prev, academic_year: val} : null)} 
              />
              <div className="pt-4 flex gap-3">
                <button onClick={() => setEditingStudent(null)} className="flex-1 px-4 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-xl font-medium transition-colors">Cancel</button>
                <button onClick={handleUpdateStudent} className="flex-1 px-4 py-2.5 bg-white text-black hover:bg-zinc-200 rounded-xl font-bold transition-colors">Save Changes</button>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative group">
        ...
        <input 
          className="w-full bg-black/40 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white text-sm focus:border-indigo-500 outline-none transition-all placeholder:text-zinc-700" 
          value={value} 
