...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen w-full bg-black text-zinc-200 font-sans flex overflow-hidden selection:bg-indigo-500/30">
      ...
      `}>
        {/* LOGO AREA */}
        <div className={`h-20 flex items-center px-6 border-b border-white/5 shrink-0 ${sidebarOpen ? 'justify-between' : 'justify-center'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
             ...
             <div className={`flex flex-col transition-all duration-300 ${!sidebarOpen && 'lg:hidden'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-3 space-y-2 custom-scrollbar">
          {NAV_ITEMS.map((item, idx) => (
             <SidebarItem 
               ...
               setSidebarOpen={setSidebarOpen} // Pass this to auto-expand on click if needed
             />
          ))}
        </div>

        {/* USER FOOTER */}
        <div className={`p-4 border-t border-white/5 bg-zinc-900/20 shrink-0 flex ${sidebarOpen ? 'justify-start' : 'justify-center'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className={`flex flex-col overflow-hidden transition-all duration-300 ${!sidebarOpen && 'lg:hidden'}`}>
        ...
        <header className="h-20 shrink-0 sticky top-0 z-30 bg-black/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center bg-zinc-900/50 border border-white/5 rounded-full px-4 py-2.5 w-80 focus-within:border-indigo-500/50 focus-within:bg-zinc-900 focus-within:ring-1 focus-within:ring-indigo-500/20 transition-all group">
               ...
               <input className="bg-transparent border-none outline-none text-sm text-zinc-200 w-full placeholder:text-zinc-600 font-medium" placeholder="Search..." />
            </div>
          </div>
          <div className="flex items-center gap-4">
...
// --- SIDEBAR ITEM (FIXED) ---
function SidebarItem({ item, expanded, onNavigate, setSidebarOpen }: any) {
    ...
    <div className="mb-1">
      <Link 
        ...
        onClick={(e) => {
          if (hasSub) {
            e.preventDefault();
            // If collapsed and clicking a parent with submenu, auto-expand sidebar
            if (!expanded && setSidebarOpen) setSidebarOpen(true);
            setIsOpen(!isOpen);
          } else {
            onNavigate(); 
          }
        ...
        className={`
          ...
          py-3 rounded-xl transition-all duration-200 cursor-pointer mx-2
          ${isActive ? 'bg-indigo-500/10 text-white' : 'text-zinc-400 hover:bg-white/5 hover:text-white'}
        `}
      >
        {/* Active Indicator */}
        ...
        <div className={`flex items-center ${expanded ? 'gap-3.5' : 'justify-center w-full'}`}>
          <item.icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-indigo-400' : 'text-zinc-500 group-hover:text-zinc-300'}`} />
          {/* Strict hiding of text when collapsed to prevent overlap */}
          <span className={`text-sm font-medium whitespace-nowrap transition-all duration-300 ${!expanded ? 'hidden' : 'block'}`}>
            {item.name}
          </span>
        </div>
      ...
      {open && (
        <div className="ml-3 mt-1 border-l border-white/10 pl-3 space-y-1">
          {item.subItems.map((sub: any, i: number) => (
             <Link key={i} href={sub.href} onClick={onNavigate} className={`block px-3 py-1.5 text-xs rounded-md transition-colors ${pathname === sub.href ? 'text-indigo-400 font-bold bg-white/5' : 'text-zinc-500 hover:text-indigo-300'}`}>
               {sub.name}
             </Link>
          ))}
        </div>
...
function LogoutButton() {
  ...
  return (
    <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 transition-all">
